class zcl_agit_lock_manager definition
  public
  final
  create private.
  ...
  public section.
    class-methods get_instance
      returning
        value(ro_instance) type ref to zcl_agit_lock_manager.
  ...
  protected section.
  private section.
    class-data go_instance type ref to zcl_agit_lock_manager. "class attribute to hold the single instance
...
class zcl_agit_lock_manager implementation.
  ...
  method is_locked.
    call function 'ENQUEUE_E_AAR_ABAPGIT'
      exporting
        mode_abapgit   = 'X'
        type           = iv_type
        value          = iv_value
    ...
    if sy-subrc <> 0.
      ...
      call function 'DEQUEUE_E_AAR_ABAPGIT'
        exporting
          mode_abapgit = 'X'
          type         = iv_type
          value        = iv_value.
