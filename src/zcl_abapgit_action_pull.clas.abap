CLASS zcl_abapgit_action_pull DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_abapgit_action_pull IMPLEMENTATION.


  METHOD zif_abapgit_action_entity~execute.
  ...
  METHOD zif_abapgit_action_entity~get_credentials.
    rs_credentials = ms_credentials.
  ENDMETHOD.


  METHOD zif_abapgit_action_entity~get_param.
  ...
  METHOD zif_abapgit_action_entity~get_paramstr.
  ...
  METHOD zif_abapgit_action_entity~get_param_type.
  ...
  METHOD zif_abapgit_action_entity~set_credentials.
    ms_credentials = is_credentials.
  ENDMETHOD.


  METHOD zif_abapgit_action_entity~set_param.
  ...
  method zif_abapgit_action_entity~initialize.
    try.
        ...
        " Explicitly acquiring lock on the repo
        CALL FUNCTION 'ENQUEUE_EABAPGIT'
          EXPORTING
            mode_abapgit  = 'E'
            type           = cl_abapgit_persistence_db=>c_type_repo
            value          = mv_repo_key
          EXCEPTIONS
            foreign_lock   = 1
            system_failure = 2
  ...
  method zif_abapgit_action_entity~cleanup.
  " Explicitly removing lock from the repo
    CALL FUNCTION 'DEQUEUE_EABAPGIT'
          EXPORTING
            mode_abapgit  = 'E'
            type           = cl_abapgit_persistence_db=>c_type_repo
            value          = mv_repo_key
  ...
  METHOD zif_abapgit_action_entity~set_paramstr.
    TRY.
        CALL TRANSFORMATION id
  ...
  METHOD zif_abapgit_action_entity~set_repo_key.
